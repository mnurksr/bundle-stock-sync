...
import { useLoaderData, useActionData, useNavigate, useSubmit, useFetcher } from "@remix-run/react";
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  if (!shop) return redirect("/app/rules");

  const rule = await db.bundleRule.findUnique({
    where: { id: params.id, shopId: shop.id },
    include: { items: true },
  });

  if (!rule) return redirect("/app/rules");
...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  if (!existingRule) return { error: "Rule not found" };
  ...
  await db.bundleRule.update({
    ...
    data: {
      ...
      bundleSku,
      items: {
        deleteMany: {}, // Delete all existing items and recreate
...
export default function EditBundleRulePage() {
  ...
  const [bundleProduct, setBundleProduct] = useState<SelectedProduct | null>({
    ...
    variants: [{ id: rule.bundleVariantId, title: "", sku: rule.bundleSku || "", displayName: "", inventoryQuantity: 0 }],
  });

  // Pre-fill items
  const initialItems: BundleItem[] = rule.items.map(item => ({
    id: item.baseProductId,
    title: item.baseProductTitle,
    variants: [{ id: item.baseVariantId, title: "", sku: item.baseSku || "", displayName: "", inventoryQuantity: 0 }],
    quantity: item.quantity,
  }));
  ...
  return (
    <Page backAction={{ content: t("rules_title"), url: "/app/rules" }} title={"Edit Rule"}>
